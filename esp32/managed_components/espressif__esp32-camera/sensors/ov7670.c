...
static int set_brightness(sensor_t *sensor, int value)
{
    // OV7670 Brightness register 0x55
    // Value is 8-bit signed
    return SCCB_Write(sensor->slv_addr, 0x55, (uint8_t)value);
}

static int set_contrast(sensor_t *sensor, int value)
{
    // OV7670 Contrast register 0x56
    // Default is 0x40
    return SCCB_Write(sensor->slv_addr, 0x56, (uint8_t)value);
}

static int set_saturation(sensor_t *sensor, int value)
{
    // OV7670 Saturation is controlled by SATCTR (0xC9)
    // Here we just write the value directly for tuning
    return SCCB_Write(sensor->slv_addr, 0xC9, (uint8_t)value);
}
