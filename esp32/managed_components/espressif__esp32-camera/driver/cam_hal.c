...
volatile uint32_t g_cam_fb_drop_cnt = 0;
...
/* Define CAM_PSRAM_LEN_CHECK to enable PSRAM non-JPEG length check. */
...
#if defined(CAM_PSRAM_LEN_CHECK)
                                if (cam_obj->frame_copy_cnt > 0) {
                                    unsigned expected_cnt = (unsigned)(cam_obj->frame_copy_cnt - 1);
                                    if ((unsigned)cnt != expected_cnt) {
