...
- 在 50FPS 采集目标下，原始标量流程无法达到50Hz的目标，且会长期占用 CPU 并挤压 UART 与 LCD 任务时间片，导致帧率下滑与系统抖动。
...
- 优化后可稳定支撑高帧率处理链路（稳定达到 50Hz 目标），同时该任务占用CPU仅 **23%**，给 `communicate_task` 与 `lcd_task` 留出更稳定的调度余量。
